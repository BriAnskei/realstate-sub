...
export class ReserveRepo {
    ...
    async create (
        ...
        const reserveResult = await this.db.run(
            ...
            `,
            [
                reservation.applicationId ? JSON.stringify(reservation.applicationId) : null,
    ...
    async findById (id: number): Promise<ReserveType | undefined> {
        ...
        return {
            ...row,
            applicationId: row.applicationId ? JSON.parse(row.applicationId as unknown as string) : [],
        };
    }
    ...
    async update (id: number, updates: Partial<Omit<ReserveType, 'id' | 'createdAt'>>): Promise<ReserveType | undefined> {
        ...
        if(updates.applicationId !== undefined) {
            ...
            values.push(updates.applicationId ? JSON.stringify(updates.applicationId) : null);
        }
